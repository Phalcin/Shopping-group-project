...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    ...
    const response = await fetch("https://aamshop.netlify.app/api/checkout", {
      method: "POST",
    ...
    <>
      {productData.length > 0 ? (
        <div className="mt-5 flex flex-col">
              ...
              {productData.map((item: ProductType) => (
                <tbody key={item?._id}>
                  <tr className="bg-white border-b-[1px] border-b-zinc-300">
                    <th
                      scope="row"
                      className="px-6 py-4 flex items-center gap-3 mr-12 block"
                    >
                      <X
                        onClick={() => {
          ...
          <button
            onClick={handleReset}
            className="bg-zinc-950 text-zinc-200 w-full py-3 mt-5 rounded-md uppercase text-xs font-semibold hover:bg-red-700 hover:text-white duration-200"
          >
            Reset Cart
          </button>
